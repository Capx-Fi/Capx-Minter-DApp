...
export default function Configuration({
  ...
  const featuresToDisplay = tokenTypeData.find((item) => {
  ...
  useEffect(() => {
    if (isNaN(userData?.initialSupply)) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      isNaN(parseFloat(userData?.taxFeePercentage))
    ) {
      valid = false;
    }
    if (
      featuresToDisplay.substring(3, 4) === "1" &&
      isNaN(parseFloat(userData?.burnFeePercentage))
    ) {
      valid = false;
    }
    if (
        ...
        if (!isNaN(toSet)) {
          if (toSet >= 0 && toSet < 100 && toSet === parseInt(toSet)) {
            if (toSet === 0) {
              ...
              setErrors({
                ...errors,
                [name]: "Must be less than total supply",
              });
              setUserData({ ...userData, [name]: toSet });
            } else {
              setErrors({ ...errors, [name]: "" });
        ...
        if (!isNaN(toSet)) {
          if (
            ...
            if (toSet === 0) {
              ...
              setErrors({
                ...errors,
                [name]: "Must be greater than or equal to initial supply",
              });
              setUserData({ ...userData, [name]: toSet });
            } else {
              setErrors({ ...errors, [name]: "" });
          ...
          <span
            ...
            dangerouslySetInnerHTML={{
              __html:
                userData?.initialSupply?.length === 0 ||
                errors?.initialSupply?.length > 0
                  ? "Please enter the initial supply first"
                  : errors?.totalSupply?.length > 0
                  ? errors.totalSupply
                  : `${
                      isNaN(parseFloat(userData?.totalSupply))
